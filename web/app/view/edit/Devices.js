...
Ext.define('Traccar.view.edit.Devices',
		{
			...
			requires : [ 'Traccar.AttributeFormatter',
			...
			viewConfig : {
				...
				getRowClass : function(record) {
					...
					if (status) {
						result += Ext.getStore('DeviceStatuses')
								.getById(status).get('color');
					}
					return result;
				}
			},
			...
			columns : {
				...
				items : [
						...
						},
						{
							...
							renderer : function(value, metaData, record,
								...
								var axle;
								var attrAxle = record.get('attributes');
								axle = attrAxle.axle;
								if (!axle) {
									axle = 'UnSpecified';
								}
						...
						{
							text : 'Load Status',
							// dataIndex: 'attributes.load',
							renderer : function(value, metaData, record,
									rowIdx, colIdx, store, view) {
								var load;
								var attrLoad = record.get('attributes');
								load = attrLoad.load;
								if (load != 0 && load != 1) {
									load = 'unknown';
								}
								var status = Ext.getStore('LoadStatuses')
										.getById(load);
								if (status) {
									metaData.style = 'font-weight: bold; color:'
											+ status.get('color') + ';';
				...
				// dataIndex: 'disabled',
				// renderer:
				// Traccar.AttributeFormatter.getFormatter('disabled'),
				// hidden: true,
				// filter: 'boolean'
				// }, {
				...
				// name =
				// Traccar.AttributeFormatter.geofenceIdFormatter(value[i]);
				// if (name) {
				...
				// renderer:
				// Traccar.AttributeFormatter.getFormatter('lastUpdate'),
				// hidden: true,
				// }
				]
			}
